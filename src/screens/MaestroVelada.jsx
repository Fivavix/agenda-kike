    ...
    <div className="animate-fade-in" style={{ padding: '24px', paddingBottom: '80px' }}>
      ...
      <div>
        <h3 style={{ fontSize: '1.05rem', marginBottom: '20px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          {viewCompleted ? 'Tickets Completados' : 'Tickets Pendientes'}
        </h3>
        
        {visibleTickets.length === 0 && (
          <div className="card text-center" style={{ padding: '40px 20px' }}>
            No hay tickets {viewCompleted ? 'completados' : 'pendientes'}
          </div>
        )}
          ...
          <div>
            {Object.entries(completedTickets.reduce((acc, t) => {
              ...
              <div key={date} style={{ marginBottom: '32px' }}>
                ...
                {tks.map(ticket => (
                  <div key={ticket.id} className="card ticket-card" style={{ opacity: 0.85 }}>
          ...
          tickets.map(ticket => (
            <div key={ticket.id} className="card ticket-card">
              ...
              <div className="note-block">
                <span style={{ color: 'var(--gold-accent)', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: '4px', fontWeight: '600' }}>Notas:</span>
                {ticket.notes}
              </div>
              ...
              <div style={{ marginTop: '20px' }}>
                ...
                {ticket.velas.map(vela => (
                  <label key={vela.id} style={{ 
                    ...
                    marginBottom: '8px', cursor: viewCompleted ? 'default' : 'pointer', 
                    ...
                    <input 
                      ...
                      onChange={() => { if (!viewCompleted) toggleVela(ticket.id, vela.id, vela.is_completed); }}
